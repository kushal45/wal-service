...
import { RedisProducerService } from '../src/modules/producers/redis/redis-producer.service';
...
describe('RedisProducerService (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const configService = {
      get: () => ({ host: 'localhost', port: 6379, db: 0 }),
  ...
  it('should handle delayed message scheduling', async () => {
    ...
    const result = await service.send(message);
    expect(result.success).toBe(true);
    // Check that delayed message is scheduled
